...
local function OnUpdate(self, dt, applyhealthdelta)
    ...
    if self.inst.components.areaaware and self.inst.components.areaaware:CurrentlyInTag("frost") then
        ambient_temperature = -20
    elseif TheWorld.state.iswinter and TUNING.tropical.kindofworld == 10 or
    ...
    if (TheWorld.state.issummer and self.inst and self.inst:HasTag("player") and TUNING.tropical.hayfever == 10 and (self.inst.components.areaaware and self.inst.components.areaaware:CurrentlyInTag("hamlet") or interior)) or
        ...
        self.inst:RemoveTag("hayfever1")
        self.inst:RemoveTag("hayfever2")
        self.inst:RemoveTag("hayfever3")
        self.inst:RemoveTag("hayfever4")
        if self.hayfever < 3000 and self.hayfever > 2000 then
            self.inst:AddTag("hayfever1")
        elseif self.hayfever > 3250 then
            self.inst:AddTag("hayfever2")
        elseif self.hayfever > 3300 then
            self.inst:AddTag("hayfever3")
        elseif self.hayfever > 3350 then
            self.inst:AddTag("hayfever4")
        end

        if self.hayfever > 3400 then
        ...
        if self.hayfever > 0 then self.hayfever = self.hayfever - 0.25 end
    end

    if self.hayfever < 2000
        or not TheWorld.state.issummer
    then
        self.inst:RemoveTag("hayfever1")
        self.inst:RemoveTag("hayfever2")
        self.inst:RemoveTag("hayfever3")
