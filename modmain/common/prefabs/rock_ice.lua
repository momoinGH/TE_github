...
local TryStageChange = Hooks.GetUpValue(rock_ice_fn, "ontimerdone", "TryStageChange")
if not TryStageChange then
    print("获取rock_ice预制件的TryStageChange函数失败，无法修改迷你冰川季节影响。")
    return
end

local SetStage = Hooks.GetUpValue(TryStageChange, "SetStage")
local RescheduleTimer = Hooks.GetUpValue(TryStageChange, "RescheduleTimer")
