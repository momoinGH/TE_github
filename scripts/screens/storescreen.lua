...
function StoreScreen:SetWidgetsPosition(perpage, perline, xspace, yspace)
    ...
    for k, widget in pairs(self.slots:GetChildren()) do
        ...
        local pos = Vector3(spacex * (col - perline / 2 - 0.5), 0.125 * self.scrnh + spacey * (lines / 2 + 0.5 - row) -
        140, 0)
...
function StoreScreen:OnUpdate()
    ...
    if self.page == 1 then
        self.button_lastpage:Disable()
    else
        self.button_lastpage:Enable()
    end

    if self.page == self.maxpage then
        self.button_nextpage:Disable()
    else
        self.button_nextpage:Enable()
    end
end
