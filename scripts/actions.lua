...
SHOP.str = (GLOBAL.STRINGS.ACTIONS.SHOP)
SHOP.id = "SHOP"
...
BOATMOUNT.str = (GLOBAL.STRINGS.ACTIONS.BOATMOUNT) --登船
...
AddComponentAction(
    ...
    function(inst, doer, actions, right)
        ...
        if right and doer:HasTag("aquatic") and containedsail and containedsail.replica.container and containedsail.replica.container:GetItemInSlot(2) ~= nil and containedsail.replica.container:GetItemInSlot(2):HasTag("sw_boatcannon") and
    ...
    GLOBAL.ActionHandler(GLOBAL.ACTIONS.ATTACK,
        function(inst, action)
            ...
            if not (inst.sg:HasStateTag(attack_tag) and action.target == inst.sg.statemem.attacktarget or inst.components.health:IsDead()) then
                ...
                if weapon and weapon:HasTag("beegun") then if inst.sg.laststate.name == "beegun" or inst.sg.laststate.name == "beegun_short" then return "beegun_short" else return
                        "beegun" end end
    ...
    GLOBAL.ActionHandler(GLOBAL.ACTIONS.ATTACK,
        function(inst, action)
            if not (inst.sg:HasStateTag("attack") and action.target == inst.sg.statemem.attacktarget or inst.replica.health:IsDead()) then
                ...
                ---------umcompromissing mode compatibility---------	
                if equip and equip:HasTag("beegun") then if inst.sg.laststate.name == "beegun" or inst.sg.laststate.name == "beegun_short" then return "beegun_short" else return
                        "beegun" end end
...
local boatrunstartcliente =
    GLOBAL.State {
        ...
        onenter = function(inst)
            ...
            inst.components.locomotor:RunForward()
            if inst:HasTag("aquatic") then
                if inst:HasTag("surf") then inst.AnimState:PlayAnimation("surf_pre") else if inst:HasTag("sail") then inst.AnimState:PlayAnimation("sail_pre") else inst.AnimState
                            :PlayAnimation("row_pre") end end
            else
                if inst.sg.statemem.normalwonkey and inst.components.locomotor:GetTimeMoving() >= TUNING.WONKEY_TIME_TO_RUN then
...
local boatrunstopcliente =
    GLOBAL.State {
        ...
        onenter = function(inst)
            ...
            inst.components.locomotor:Stop()
            if inst:HasTag("aquatic") then
                if inst:HasTag("surf") then inst.AnimState:PlayAnimation("surf_pst") else if inst:HasTag("sail") then inst.AnimState:PlayAnimation("sail_pst") else inst.AnimState
                            :PlayAnimation("row_pst") end end
            else
                inst.AnimState:PlayAnimation(GetRunStateAnim(inst) .. "_pst")
