...
function FN.FnDecorator(obj, key, beforeFn, afterFn, isUseBeforeReturn)
    if not prosoftassert(type(obj) == "table")
        or not prosoftassert(beforeFn == nil or type(beforeFn) == "function", "beforeFn must be nil or a function")
        or not prosoftassert(afterFn == nil or type(afterFn) == "function", "afterFn must be nil or a function") then
        return
    end
