...
local ANY_TOWORK_ACTIONS = { ACTIONS.CHOP, ACTIONS.MINE, ACTIONS.DIG, ACTIONS.HACK }
local ANY_TOWORK_MUSTONE_TAGS = { "CHOP_workable", "MINE_workable", "DIG_workable", "machetecut" }
...
local function FindAnyEntityToWorkActionsOn(inst, ignorethese) -- This is similar to FindEntityToWorkAction, but to be very mod safe FindEntityToWorkAction has been deprecated.
    ...
    local target = FilterAnyWorkableTargets(
    TheSim:FindEntities(px, py, pz, TUNING.SHADOWWAXWELL_WORKER_WORK_RADIUS_LOCAL, nil, TOWORK_CANT_TAGS,
        ANY_TOWORK_MUSTONE_TAGS), ignorethese, leader, inst)
    if target ~= nil then
    ...
    if target == nil then
        target = FilterAnyWorkableTargets(
        TheSim:FindEntities(spawn.x, spawn.y, spawn.z, TUNING.SHADOWWAXWELL_WORKER_WORK_RADIUS, nil, TOWORK_CANT_TAGS,
            ANY_TOWORK_MUSTONE_TAGS), ignorethese, leader, inst)
    end
...
function ShadowWaxwellBrain:OnStart()
    ...
    local avoid_danger = RunAway(self.inst,
        { fn = ShouldRunAway, oneoftags = { "monster", "hostile" }, notags = { "player", "INLIMBO", "companion", "spiderden" } },
    ...
    elseif self.inst.prefab == "shadowprotector" then
        root = PriorityNode({ -- This protector is set to defend an area and then vanish.
            ...
            -- Leashing is low priority.
            Leash(self.inst, GetSpawn, math.min(8, TUNING.SHADOWWAXWELL_PROTECTOR_DEFEND_RADIUS),
        ...
        root = PriorityNode({
            ...
            WhileNode(function() return IsNearLeader(self.inst, KEEP_WORKING_DIST) end, "Leader In Range",
                PriorityNode({
                    ...
                    WhileNode(
                        function() return self.inst.prefab == "shadowlumber" and not self.inst.sg:HasStateTag("phasing") end,
                        "Keep Chopping",
                    ...
                    WhileNode(
                        function() return self.inst.prefab == "shadowminer" and not self.inst.sg:HasStateTag("phasing") end,
                        "Keep Mining",
                        DoAction(self.inst, function() return FindEntityToWorkAction(self.inst, ACTIONS.MINE) end)),
                    WhileNode(
                        function() return self.inst.prefab == "shadowdigger" and not self.inst.sg:HasStateTag("phasing") end,
                        "Keep Digging",
                        DoAction(self.inst,
