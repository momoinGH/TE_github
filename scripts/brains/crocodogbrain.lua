...
local function HarvestAction(inst)
    local target = FindEntity(inst, SEE_DIST,
        function(item) return item.components.breeder and item.components.breeder.volume > 0 end)
    if target then
...
local function EatFoodAction(inst)
    local target = FindEntity(inst, SEE_DIST,
        function(item) return inst.components.eater:CanEat(item) and item:IsOnValidGround() end)
    if target then
    ...
    local root = PriorityNode(
        {
            ...
            WhileNode(function() return not self.inst:HasTag("pet_hound") and GetHome(self.inst) end, "No Pet Has Home",
                ChaseAndAttack(self.inst, 10, 20)),
            WhileNode(function() return not self.inst:HasTag("pet_hound") and not GetHome(self.inst) end, "Not Pet",
