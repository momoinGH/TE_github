    ...
    local root = PriorityNode(
        {
            WhileNode(function() return not self.inst:HasTag("cocoon") end, "is not cocoon",
                PriorityNode {
                    ...
                    WhileNode(function() return self.inst:HasTag("wantstococoon") and not self.inst.onwater end, "do cocoon", ActionNode(function()
                        startCocooning(self.inst) end)),

                    WhileNode(function() return self.inst.components.combat.target and
                        self.inst.components.combat:InCooldown() end, "Dodge", RunAway(self.inst, function() return self
                        .inst.components.combat.target end, RUN_AWAY_DIST, STOP_RUN_AWAY_DIST)),
