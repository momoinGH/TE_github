...
--            IfNode(function() return self.inst.yotb_post_to_mark end, "mark post",
    ...
    local day = WhileNode( function() return TheWorld.state.isday end, "IsDay",
        PriorityNode{
            ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_FIND_MEAT,
                DoAction(self.inst, FindFoodAction )),
            IfThenDoWhileNode(function() return StartChoppingCondition(self.inst) end, function() return KeepChoppingAction(self.inst) end, "chop",
                    LoopNode{ 
                        ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_HELP_CHOP_WOOD,
            ...
            --[[IfNode(function() return StartHackingCondition(self.inst) end, "hack", 
                WhileNode(function() return KeepHackingAction(self.inst) end, "keep hacking",
                    LoopNode{ 
                        ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_HELP_HACK,
                            DoAction(self.inst, FindBushToHackAction ))})),]]
            ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_FOLLOWWILSON, 
            ...
            ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_RUNAWAY_WILSON,
                    RunAway(self.inst, "player", START_RUN_DIST, STOP_RUN_DIST)),
                    --DoAction(self.inst, GetAnnoyedFn),
            ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_LOOKATWILSON,
    ...
    local night = WhileNode( function() return not TheWorld.state.isday end, "IsNight",
        PriorityNode{
            ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_RUN_FROM_SPIDER,
            ...
            ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_GO_HOME,
            ...
            ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_FIND_LIGHT,
                FindLight(self.inst, SEE_LIGHT_DIST, SafeLightDist)),
            ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_PANIC,
                Panic(self.inst)),
        }, 1)
    ...
    local root =
        PriorityNode(
        {
            ...
            WhileNode(function() return self.inst.components.health.takingfiredamage end, "OnFire",
				ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_PANICFIRE,
					Panic(self.inst))),
            BrainCommon.IpecacsyrupPanicTrigger(self.inst),
            ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_FIGHT,
            ...
            ChattyNode(self.inst, STRINGS.SWAMPIG_TALK_ATTEMPT_TRADE,
