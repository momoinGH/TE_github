...
self.buffs[c]={}
for e,f in pairs(d)do if not self.data[e]then self.data[e]={}end;
self.data[e][c]=f;
table.insert(self.buffs[c],e)end end;

function a:GetBuffData(e)
local f=0;
if self:HasBuffData(e)
then for g,h in pairs(self.data[e])do 
f=f+h end end;
return f 
end;

function a:HasBuff(c)
return self.buffs[c]~=nil end;

function a:HasBuffData(c)
return self.data[c]~=nil end;
