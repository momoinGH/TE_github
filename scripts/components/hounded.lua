...
return Class(function(self, inst)
    ...
    local _spawndata = {base_prefab = "hound", winter_prefab = "icehound", summer_prefab = "firehound",
                        upgrade_spawn = "warglet",

                        attack_levels = {intro = {warnduration = function() return 120 end,
                                                  numspawns = function() return 1 end},
                                         light = {warnduration = function() return 60 end,
                                                  numspawns = function() return 2 + math.random(2) end},
                                         med = {warnduration = function() return 45 end,
                                                numspawns = function() return 3 + math.random(3) end},
                                         heavy = {warnduration = function() return 30 end,
                                                  numspawns = function() return 4 + math.random(3) end},
                                         crazy = {warnduration = function() return 30 end,
                                                  numspawns = function() return 6 + math.random(4) end}},
    ...
    -- these names are describing the strength of the houndwave more than the duration
                        attack_delays = {intro = function()
        return TUNING.TOTAL_DAY_TIME * 5, math.random() * TUNING.TOTAL_DAY_TIME * 3
    end, light = function() return TUNING.TOTAL_DAY_TIME * 5, math.random() * TUNING.TOTAL_DAY_TIME * 5 end,
                                         med = function()
        return TUNING.TOTAL_DAY_TIME * 7, math.random() * TUNING.TOTAL_DAY_TIME * 5
    end, heavy = function() return TUNING.TOTAL_DAY_TIME * 9, math.random() * TUNING.TOTAL_DAY_TIME * 5 end,
                                         crazy = function()
        return TUNING.TOTAL_DAY_TIME * 11, math.random() * TUNING.TOTAL_DAY_TIME * 5
    end}, warning_speech = "ANNOUNCE_HOUNDS", warning_sound_thresholds = { -- Key = time, Value = sound prefab
    {time = 30, sound = "LVL4"}, {time = 60, sound = "LVL3"}, {time = 90, sound = "LVL2"}, {time = 500, sound = "LVL1"}},
    ...
    local function CalcPlayerAttackSize(player)
        ...
        return (day < 10 and _spawndata.attack_levels.intro.numspawns()) or
                   (day < 25 and _spawndata.attack_levels.light.numspawns()) or
                   (day < 50 and _spawndata.attack_levels.med.numspawns()) or
                   (day < 100 and _spawndata.attack_levels.heavy.numspawns()) or
                   _spawndata.attack_levels.crazy.numspawns()
    end
        ...
        for player, group in pairs(groupindex) do
            ...
            if spawnsToRelease > 0 then
                if groupmap[group] == nil then
                    ...
                    table.insert(_spawninfo, {players = {}, -- tracks the number of spawns for this player
                    timetonext = 0, -- working data
                    target_weight = {}, spawnstorelease = 0, totalplayerage = 0})
                end
                ...
                _spawninfo[g].target_weight[player] = math.sqrt(spawnsToRelease) *
                                                          (player.components.houndedtarget ~= nil and
                                                              player.components.houndedtarget:GetTargetWeight() or 1)
    ...
    local function GetDelayedPlayerWaveAmounts(player, data)
        ...
        table.insert(data._spawninfo,
            ...
            timetonext = 0, averageplayerage = playerAge})
    end
    ...
    local function GenerateSaveDataFromDelayedSpawnInfo(player, savedata, delayedspawninfo)
        savedata[player.userid] = {_warning = delayedspawninfo._warning, _timetoattack = delayedspawninfo._timetoattack,
        ...
        if delayedspawninfo._spawninfo then
            ...
            savedata[player.userid]._spawninfo = {count = spawninforec.players and spawninforec.players[player] or 0,
    ...
    local function GenerateSaveDataFromSpawnInfo(player, savedata)
        savedata[player.userid] = {_warning = _warning, _timetoattack = _timetoattack, _warnduration = _warnduration,
        ...
        if _spawninfo then
            for i, spawninforec in ipairs(_spawninfo) do
                if spawninforec.players[player] then
                    savedata[player.userid]._spawninfo = {count = spawninforec.players[player],
                                                          timetonext = spawninforec.timetonext,
                                                          averageplayerage = spawninforec.averageplayerage}
                    break
                end
    ...
    local function LoadSaveDataFromMissingSpawnInfo(player, missingspawninfo)
        ...
        _delayedplayerspawninfo[player] = {_warning = missingspawninfo._warning,
        ...
        if missingspawninfo._spawninfo then
            ...
            _delayedplayerspawninfo[player]._spawninfo = {players = {[player] = spawninforec.count},
                                                          timetonext = spawninforec.timetonext,
                                                          averageplayerage = spawninforec.averageplayerage}
        end
    end
    ...
    function self:GetTimeToAttack() return _timetoattack end

    function self:GetWarning() return _warning end

    function self:GetAttacking() return ((_timetoattack <= 0) and _attackplanned) end

    function self:SetSpawnData(data) _spawndata = data end
    ...
    function self:SetSummerVariant(enabled) _spawnsummervariant = enabled end

    function self:SetWinterVariant(enabled) _spawnwintervariant = enabled end

    function self:SetWaveOverrideSettings(data) _wave_override_settings[data.wavetype] = data.setting end
    ...
    function self:SummonSpawn(pt, radius_override) return pt ~= nil and SummonSpawn(pt, nil, radius_override) or nil end
    ...
    local function HandleSpawnInfoRec(dt, i, spawninforec, groupsdone)
        ...
        if next(spawninforec.players) ~= nil and spawninforec.timetonext < 0 then
            ...
            if _spawndata.upgrade_spawn and _spawndata.ShouldUpgrade then
                ...
                upgrade, houndcount, local_wave_pre_upgraded =
                    _spawndata.ShouldUpgrade(spawninforec.players[target], local_wave_pre_upgraded)
    ...
    function self:OnUpdate(dt)
        ...
        for player, data in pairs(_delayedplayerspawninfo) do
            ...
            if data._warning then
                ...
                if data._timetonextwarningsound <= 0 then
                    ...
                    data._timetonextwarningsound = (data._timetoattack < 30 and .3 + math.random(1)) or
                                                       (data._timetoattack < 60 and 2 + math.random(1)) or
                                                       (data._timetoattack < 90 and 4 + math.random(2)) or 5 +
                                                       math.random(4)
        ...
        if _warning then
            ...
            if _timetonextwarningsound <= 0 then
                ...
                _timetonextwarningsound = (_timetoattack < 30 and .3 + math.random(1)) or
                                              (_timetoattack < 60 and 2 + math.random(1)) or
                                              (_timetoattack < 90 and 4 + math.random(2)) or 5 + math.random(4)
    ...
    function self:OnSave()
        ...
        return {warning = _warning, timetoattack = _timetoattack, warnduration = _warnduration,
                attackplanned = _attackplanned, missingplayerspawninfo = missingspawninfo,
                wave_override_chance = _wave_override_chance, wave_pre_upgraded = _wave_pre_upgraded}
    end
    ...
    function self:GetDebugString()
        if _timetoattack > 0 then
            return string.format("%s spawns are coming in %2.2f  _wave_override_chance: %2f, _wave_pre_upgraded: %s",
                (_warning and "WARNING") or (_pausesources:Get() and "BLOCKED") or "WAITING", _timetoattack,
                _wave_override_chance, _wave_pre_upgraded or "NO")
        end
        ...
        if _spawnmode ~= "never" then
            ...
            for i, spawninforec in ipairs(_spawninfo) do
                ...
                s = s .. "} - spawns left:" .. tostring(spawninforec.spawnstorelease) .. " next spawn:" ..
                        tostring(spawninforec.timetonext)
            end
        end
