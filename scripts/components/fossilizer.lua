...
function Fossilizer:SpawnFosilFx(d)
    local e = {}
    for f = 1, 20 do
        local g = f / 5; local h = math.sqrt(g * 512)
        local i = math.sqrt(g)
        local j = Vector3(math.sin(h) * i * 1.2, 0, math.cos(h) * i * 1.2)
        local k = TheWorld.Map:GetTileAtPoint((d + j):Get())
        if k ~= 1 and k ~= 255 then
            table.insert(e, j)
        end
    end;
    local f = 0; self.spawn_task = self.inst:DoPeriodicTask(.15, function()
        f = f + 1; if f >= 13 and self.spawn_task then
            self.spawn_task:Cancel()
            self.spawn_task = nil;
            return
        end;
        SpawnAt("fossilizing_fx", d + e[math.random(#e)]).Transform:SetScale(.65, .65, .65)
    end)
end;
...
function Fossilizer:OnRemoveEntity()
    for n, p in ipairs({ "spawn_task", "check_task" }) do
        if self[p] ~= nil then
            self[p]:Cancel()
            self[p] = nil
        end
    end; if self.inst.components.reticule_spawner then
        self.inst.components.reticule_spawner:KillRet()
    end
end;

Fossilizer.OnRemoveFromEntity = Fossilizer.OnRemoveEntity; function Fossilizer:GetDebugString()
    return string.format("Duration: %2.2f",
        self.duration)
end;
