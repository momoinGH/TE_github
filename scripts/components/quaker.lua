...
return Class(function(self, inst)
    ...
    local _miniquakesoundintensity = net_bool(inst.GUID, "quaker._miniquakesoundintensity",
        "miniquakesoundintensitydirty")
    ...
    local GetSpawnPoint = _ismastersim and function(pt, rad, minrad)
        ...
        if result_offset ~= nil then
            ...
            local num_ents = TheSim:CountEntities(newpt.x, newpt.y, newpt.z, TUNING.QUAKE_BLOCKER_RANGE,
                QUAKE_BLOCKER_MUST_TAGS)
    ...
    local DoDropForPlayer = _ismastersim and function(player, reschedulefn)
        ...
        local riftspawner = _world.components.riftspawner
        if riftspawner and riftspawner:IsShadowPortalActive() and math.random() < TUNING.RIFT_SHADOW1_QUAKER_ODDS then
            local ents = TheSim:FindEntities(px, py, pz, TUNING.RIFT_SHADOW1_QUAKER_RADIUS, nil, STRUCTURES_CANT_TAGS,
                STRUCTURES_ONEOF_TAGS)
            if ents[1] == nil then
                override_prefab = "cavein_boulder"
                rad = TUNING.RIFT_SHADOW1_QUAKER_RADIUS
                override_density = 0
