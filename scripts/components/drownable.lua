...
function Drownable:IsOverWater()
	...
	local x, y, z = self.inst.Transform:GetWorldPosition()
	return not TileGroupManager:IsInvalidTile(TheWorld.Map:GetTileAtPoint(x, y, z)) -- allow players to be out of bounds so that a number of mods will still work
		and TheWorld.Map:IsOceanTileAtPoint(x, y, z)
end
