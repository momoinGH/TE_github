...
function Driver:OnUpdate(dt) --Set my entity's position and rotation to be the same as the drivable entity's
    ...
    --     if not self.inst.components.interactions then
    --         self.inst:AddComponent("interactions")
    --     end
    ...
    if gastabarco then
        if gastabarco.components.container and not gastabarco.components.container:IsOpen() and self.inst.sg:HasStateTag("sailing") then
            gastabarco.components.container:Open(self
                .inst)
        end
        ...
        if gastabarco.components.armor and gastabarco.components.armor.condition > self.vehicle.components.finiteuses.current then
            gastabarco.components.armor.condition = self
                .vehicle.components.finiteuses.current
        end
        if gastabarco.components.armor and gastabarco.components.armor.condition < self.vehicle.components.finiteuses.current then
    ...
    if gastabarco and gastabarco.components.container then
        ...
        if luzslot ~= nil and self.luzequipada ~= nil and luzslot.prefab ~= self.luzequipada.prefab then
            ...
            --     self.inst.AnimState:OverrideSymbol(luzslot.symboltooverride, luzslot.build, luzslot.symbol)
            self.vehicle.AnimState:OverrideSymbol(luzslot.symboltooverride, luzslot.build, luzslot.symbol)
            self.inst.AnimState:OverrideSymbol("swap_sail", "swap_sail", "swap_sail")
            self.simbolo1 = luzslot.symboltooverride
            self.luzequipada = luzslot
        ...
        if sailslot ~= nil and self.vehicle ~= nil and sailslot.prefab == "trawlnet" then
            ...
            if ground and self.inst.components.locomotor.isrunning and (ground == GROUND.OCEAN_WATERLOG or ground == GROUND.OCEAN_COASTAL_SHORE or ground == GROUND.OCEAN_SWELL or ground == GROUND.OCEAN_BRINEPOOL or ground == GROUND.OCEAN_COASTAL) then
                sailslot.raso =
                    sailslot.raso + 0.003
            end
...
function Driver:OnMount(vehicle)
    ...
    -- 重新生成一个船跟随玩家？
    local boat = SpawnPrefab(vehicle.prefab)
    boat.components.finiteuses.current = vehicle.components.finiteuses.current
