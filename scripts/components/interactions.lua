...
function Interactions:Jump(jumper)
	if jumper and jumper:HasTag('player') then
		...
		local local_passable = TheWorld.Map:IsPassableAtPoint(x3, 0, z3) and TheWorld.Map:IsPassableAtPoint(x5, 0, z5) and TheWorld.Map:IsPassableAtPoint(x6, 0, z6) and
			TheWorld.Map:IsPassableAtPoint(x7, 0, z7)
		...
		if dist <= reach_dist then
			...
			if not victim:HasTag('player') and victim.components.health and victim.components.combat then
				...
				victim.components.combat:ShareTarget(self.inst, 15, function(dude)
					return dude.prefab and victim.prefab and dude.prefab == victim.prefab and not (dude.components.follower and dude.components.follower.leader == self.inst)
				end, 1)
