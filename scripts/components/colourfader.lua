...
local Colourfader = Class(function(self, d)
    self.inst = d;
...
function Colourfader:EndFade()
    ...
    self.time = nil;
    if self.callback then self.callback(self.inst) end
end;

function Colourfader:StartFade(e, f, g)
    self.callback = g;
    self.target_colour = e;
    self.time = f;
    self.timepassed = 0;
    if self.time > 0 then
...
function Colourfader:SetCurrentFade(e)
    self.current_colour = e
end;

function Colourfader:OnUpdate(h)
    self.timepassed = self.timepassed + h;
    ...
    if self.timepassed >= self.time then self:EndFade() end
end;
