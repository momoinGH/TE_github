...
local function DefaultWhileGroggy(inst)
    local self = inst.components.grogginess
...
local function DefaultWhileWearingOff(inst)
    local self = inst.components.grogginess
...
local function DefaultOnWearOff(inst)
    local self = inst.components.grogginess
    self.speed_mod = 1
end

local function Check(inst, self)
    local check = TheWorld.state.iswinter
        and TheWorld.state.precipitationrate > 0 --正在下雪
        and (TUNING.tropical.fog == 20 or inst.components.areaaware:CurrentlyInTag("hamlet"))
        or false

    if self.last_check ~= check then
        self:SetGroggyWeather(check) --check作为起雾和结束起雾的标志
...
local Grogginess = Class(function(self, inst)
    ...
    self.last_check = nil
    inst:DoPeriodicTask(1, Check, 0, self)
