...
function Shelfer:GiveGift()
    ...
    if shelf and shelf:HasTag("pigcurse") then
        if shelf.components.timer then
            shelf.components.timer:StartTimer("spawndelay", 480 * 60)
        end
        if math.random() < 0.3 then
            local ghost = SpawnPrefab("ghost")
            local pt = Vector3(shelf.Transform:GetWorldPosition())
            if ghost then ghost.Transform:SetPosition(pt.x, pt.y, pt.z) end
        end
    end

    if shelf and shelf:HasTag("yellow_lizardman") then
        if shelf.components.timer then
            shelf.components.timer:StartTimer("spawndelay", 480 * 60)
        end
