        ...
        events =
        {
            EventHandler("animover", function(inst)
                if not inst.sg.statemem.norepeat and math.random() < .333 then
                    inst.sg:GoToState("taunt",
                        inst.components.follower.leader ~= nil and inst.components.follower.leader:HasTag("player"))
                else
                    inst.sg:GoToState("idle")
                end
            end),
            ...
            EventHandler("animover", function(inst)
                ...
                elseif inst.sg.statemem.count == 0 and math.random() < .333 then
                    inst.sg:GoToState("howl",
                        inst.components.follower.leader ~= nil and inst.components.follower.leader:HasTag("player") and
                        -1 or 0)
                else
                    inst.sg:GoToState("idle")
                end
            ...
            if inst.components.embarker:HasDestination() then
                ...
                elseif inst.sg.statemem.timeout then
                    ...
                    inst.sg:GoToState("hop_pst",
                        not TheWorld.Map:IsVisualGroundAtPoint(x, y, z) and inst:GetCurrentPlatform() == nil)
                end
