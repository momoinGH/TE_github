...
local states =
{
        ...
        events =
        {
            EventHandler("animover", function(inst)
                ...
                if target then
                    inst.sg:GoToState(
                        target:IsValid()
                        and not inst:IsNear(target, MELEE_RANGE)
                        and "attack_range"
                        or "attack",
                        target
                    )
    ...
    State {
        ...
        events =
        {
            EventHandler("animqueueover", function(inst)
                local target = inst.components.combat.target
                if target then
                    if inst:IsNear(target, MELEE_RANGE) then
                        inst.sg:GoToState("attack")
                    else
                        inst.sg:GoToState("attack_pause")
                    end
                else
                    inst.sg:GoToState("attack_post")
                end
    ...
    State {
        ...
        events =
        {
            EventHandler("animqueueover", function(inst) inst.sg:GoToState("attack_pause") end),
        },
    },
            ...
            EventHandler("animqueueover", function(inst)
                ...
                if target then
                    if inst:IsNear(target, MELEE_RANGE) then
                        ...
                        inst.sg:GoToState("attack_range")
                    end
                else
                    inst.sg:GoToState("attack_post")
                end
