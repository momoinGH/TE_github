...
local function GetTreasure(inst)
    if inst.treasure and inst.treasure:IsValid() then
        return inst.treasure
    end
...
local function onunwrapped(inst, doer)
    local treasure = GetTreasure(inst)
    local x, y, z = treasure.Transform:GetWorldPosition()
...
return Prefab("bandittreasure", fn, assets),
