...
local function OnProxyBuilt(inst)
    local x, y, z = inst.Transform:GetWorldPosition()
    local ents = TheSim:FindEntities(x, y, z, InteriorSpawnerUtils.RADIUS, inst.findTags)
    if #ents > 0 then
        local ent = ents[1]
        SpawnAt("collapse_small", ent)
        ent.initData = inst.initData
        InteriorSpawnerUtils.InitHouseInteriorPrefab(ent, ent.initData)
    end
end

-- 代理类型一，只是替换动画数据
function DecoCreator:CreateDecoProxy(name, build, bank, anim, findTags)
    local function proxyFn()
        local inst = CreateEntity()
        inst.entity:AddTransform()
        inst.entity:AddNetwork()
