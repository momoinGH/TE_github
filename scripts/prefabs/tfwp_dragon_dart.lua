...
local function l(h, m)
    if m ~= nil then
        ...
        local q = m.x - n; local r = m.z - p; local s = q * q + r * r; if s <= 0 then return h.components.reticule.targetpos end; s = 6.5 / math.sqrt(s)
...
local function F()
    ...
    h.components.weapon:SetAltAttack(TUNING.FORGE_ITEM_PACK.TFWP_DRAGON_DART.ALT_DAMAGE, { 10, 15 }, nil, DAMAGETYPES.PHYSICAL)
    return h
end;
local G = 5;
