...
local d = { "hearthsfire_crystals_projectile", "hearthsfire_crystals_proc_fx", "hearthsfire_crystals_sparks",
    "reticuleaoesmall", "reticuleaoesmallping", "reticuleaoesmallhostiletarget" }
...
local function j()
    local k = ThePlayer; local l = TheWorld.Map; local m = Vector3()
    for n = 5, 0, -.25 do
        m.x, m.y, m.z = k.entity:LocalToWorldSpace(n, 0, 0)
        if l:IsPassableAtPoint(m:Get()) and not l:IsGroundTargetBlocked(m) then return m end
    end; return m
end;
local function o(g, h)
    ...
    g.Physics:CollidesWith(COLLISION.ITEMS)
end;
local p = TUNING.FORGE_ITEM_PACK.TFWP_FIRE_BOMB.ALT_RANGE;
local function q(g, r, s)
    g.SoundEmitter:KillSound("hiss")
    SpawnPrefab("hearthsfire_crystals_explosion").Transform:SetPosition(g.Transform:GetWorldPosition())
    local t = {}
    local u, v, w = g:GetPosition():Get()
    local x = TheSim:FindEntities(u, v, w, p, nil, { "player", "companion" })
    ...
    end; if g.owner.components.weapon then g.owner.components.weapon:DoAltAttack(g.attacker, t, nil, "explosive") end;
    g:Remove()
end;
local function A(g)
    g.charge = 0; if g.charge_fx then
        g.charge_fx:Remove()
        g.charge_fx = nil
    end;
    if g.chargetask then
        g.chargetask:Cancel()
        g.chargetask = nil
    end;
    g.SoundEmitter:KillSound("hiss")
end; local function B(g, C, m)
    local D = SpawnPrefab("hearthsfire_crystals_projectile")
    D.Transform:SetPosition(g:GetPosition():Get())
    D.owner = C;
    D.components.complexprojectile:Launch(m, C)
    D:AttackArea(C, g, m)
    A(g)
    g.components.recarregavel:StartRecharge()
end;
local function E(g, F)
    if not g.charge_fx and g.components.inventoryitem.owner then
    ...
    g.charge_fx.SetSparkLevel(g.charge_fx, F)
end;
local function G(g, C, H)
    local m = H:GetPosition()
    local I = SpawnPrefab("hearthsfire_crystals_proc_fx")
    I.Transform:SetPosition(m:Get())
    I.owner = g; I.attacker = C; I.target = H; A(g)
end;
local function J(g, C, H)
    if g.components.inventoryitem.owner then
        ...
        if g.charge >= TUNING.FORGE_ITEM_PACK.TFWP_FIRE_BOMB.CHARGE_HITS_1 and g.charge < TUNING.FORGE_ITEM_PACK.TFWP_FIRE_BOMB.CHARGE_HITS_2 then
            E(g, 1)
        elseif g.charge >= TUNING.FORGE_ITEM_PACK.TFWP_FIRE_BOMB.CHARGE_HITS_2 and g.charge < TUNING.FORGE_ITEM_PACK.TFWP_FIRE_BOMB.MAXIMUM_CHARGE_HITS then
            E(g, 2)
        elseif g.charge >= TUNING.FORGE_ITEM_PACK.TFWP_FIRE_BOMB.MAXIMUM_CHARGE_HITS then
            G(g, C, H)
        end
    end
end;
...
local function K()
    ...
    g.components.weapon.onattack = J;
    g.components.weapon:SetStimuli("fire")
    g.components.weapon:SetAltAttack(TUNING.FORGE_ITEM_PACK.TFWP_FIRE_BOMB.ALT_DAMAGE, { 10, 20 }, nil,
...
local function O()
    ...
    if not TheNet:IsDedicated() then
        g.animent = M()
        g.animent.entity:SetParent(g.entity)
        if not TheWorld.ismastersim then g:ListenForEvent("directiondirty", N) end
    end;
    g.entity:SetPristine()
    if not TheWorld.ismastersim then return g end;
    g.AttackArea = function(g, r, P, m)
        P.firebomb = g;
        g.attacker = r; g.owner = P
    end;
    ...
    g.components.complexprojectile:SetOnHit(q)
    return g
end;
local function Q(g)
    g.SoundEmitter:PlaySound("dontstarve/common/blackpowder_explo")
    local t = {}
    local u, v, w = g:GetPosition():Get()
    ...
    if g.owner.components.weapon then
        g.owner.components.weapon:DoAltAttack(g.attacker, t, nil, "explosive")
    end
end;
local function R()
    local g = CreateEntity()
...
local function S()
    ...
    g.persists = false; g.AttackOther = function(P, C, s)
        g.target = s; g.attacker = C
    end;
    g:DoTaskInTime(0, Q)
    return g
end;
...
local function U()
    ...
    g.persists = false;
    g:ListenForEvent("animover", g.Remove)
    g.SetSparkLevel = T; return g
end;

return
    CustomPrefab("tfwp_fire_bomb", K, a, d, nil, "images/inventoryimages.xml", "lavaarena_firebomb.tex",
        TUNING.FORGE_ITEM_PACK.TFWP_FIRE_BOMB, "swap_lavaarena_firebomb", "common_hand"),
