...
local function dropparts(inst)
    for _, part in ipairs(parts) do
        ...
        local x, y, z = inst.Transform:GetWorldPosition()
        local offset = FindWalkableOffset(inst:GetPosition(), math.random() * TWOPI, 80, 14, true, true)
        if offset then
            x = x + offset.x
            z = z + offset.z
        end
...
local function spawnbarrier(inst, pt)
    ...
    for i = 1, number do
        ...
        if TheWorld.Map:IsLandTileAtPoint(newpt.x, newpt.y, newpt.z) then
...
local function onnearmine(inst, ents)
    ...
    if inst.primed and detonate and not inst.explode_task then
        ...
        inst.explode_task = inst:DoTaskInTime(0.8, function()
            --explode, end beep
            inst.SoundEmitter:KillSound("boom_loop")
            ShakeAllCameras(CAMERASHAKE.VERTICAL, .7, .025, 0.25, inst, SHAKE_DIST)
            inst:Hide()
