...
local function restock(inst, force)
    ...
    elseif not inst:HasTag("justsellonce") or force then
        ...
        if inst.shoptype == "pig_shop_deli" or inst.shoptype == pig_shop_deli then
            largura = 12
            inst.shoptype = pig_shop_deli
        end
        if inst.shoptype == "pig_shop_florist" or inst.shoptype == pig_shop_florist then
            largura = 15
            inst.shoptype = pig_shop_florist
        end
        if inst.shoptype == "pig_shop_general" or inst.shoptype == pig_shop_general then
            largura = 15
            inst.shoptype = pig_shop_general
        end
        if inst.shoptype == "pig_shop_hoofspa" or inst.shoptype == pig_shop_hoofspa then
            largura = 7
            inst.shoptype = pig_shop_hoofspa
        end
        if inst.shoptype == "pig_shop_produce" or inst.shoptype == pig_shop_produce then
            largura = 21
            inst.shoptype = pig_shop_produce
        end
        if inst.shoptype == "pig_shop_antiquities" or inst.shoptype == pig_shop_antiquities then
            largura = 13
            inst.shoptype = pig_shop_antiquities
        end
        if inst.shoptype == "pig_shop_cityhall" or inst.shoptype == pig_shop_cityhall then
            largura = 2
            inst.shoptype = pig_shop_cityhall
        end
        if inst.shoptype == "pig_shop_arcane" or inst.shoptype == pig_shop_arcane then
            largura = 10
            inst.shoptype = pig_shop_arcane
        end
        if inst.shoptype == "pig_shop_weapons" or inst.shoptype == pig_shop_weapons then
            largura = 10
            inst.shoptype = pig_shop_weapons
        end
        if inst.shoptype == "pig_shop_spears" or inst.shoptype == pig_shop_spears then
            largura = 15
            inst.shoptype = pig_shop_spears
        end
        if inst.shoptype == "pig_shop_hatshop" or inst.shoptype == pig_shop_hatshop then
            largura = 15
            inst.shoptype = pig_shop_hatshop
        end
        if inst.shoptype == "pig_shop_bank" or inst.shoptype == pig_shop_bank then
            largura = 3
            inst.shoptype = pig_shop_bank
        end
        if inst.shoptype == "pig_shop_tinker" or inst.shoptype == pig_shop_tinker then
            largura = 7
            inst.shoptype = pig_shop_tinker
        end
        if inst.shoptype == "pig_shop_academy" or inst.shoptype == pig_shop_academy then
            largura = 7
            inst.shoptype = pig_shop_academy
        end
        if inst.shoptype == "pig_shop_fishing" or inst.shoptype == pig_shop_fishing then
            largura = 19
            inst.shoptype = pig_shop_fishing
        end
