...
local function MakeInventoryRabbit(inst)
    inst._crazyinv = nil
    inst.components.inventoryitem:ChangeImageName(IsWinterRabbit(inst) and "rabbit_winter" or "goddess_rabbit_live")
...
local function BecomeRabbit(inst)
    ...
    inst.AnimState:SetBuild("grabit_build")
...
local function fn()
    ...
    inst.AnimState:SetClientsideBuildOverride("insane", "rabbit", "beard_monster")
...
return Prefab("goddess_rabbit", fn, assets, prefabs)
