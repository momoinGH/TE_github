-- 基本拷贝原版猪人预制件，提供一些函数调用时机用来覆盖原来参数，方便做换皮猪人
...
local function SetNormalPig(inst)
    ...
    inst:SetBrain(inst._data.brain or normalbrain)
    inst:SetStateGraph(inst._data.sg or "SGpig")
    ...
    inst.components.combat:SetTarget(nil)
    inst:RemoveEventCallback("suggest_tree_target", SuggestTreeTarget)
    inst:ListenForEvent("suggest_tree_target", SuggestTreeTarget)

    inst.components.trader:Enable()
    inst.components.talker:StopIgnoringAll("becamewerepig")

    if inst._data.SetNormalPigPost then
        inst._data.SetNormalPigPost(inst)
    end
end
...
local function SetGuardPig(inst)
    inst:RemoveTag("werepig")
    inst:AddTag("guard")
    inst:SetBrain(inst._data.brain or guardbrain)
    inst:SetStateGraph(inst._data.sg or "SGpig")
    inst.AnimState:SetBuild(inst.build)
...
local function SetWerePig(inst)
    inst:AddTag("werepig")
    inst:RemoveTag("guard")
    inst:SetBrain(inst._data.were_brain or werepigbrain)
    inst:SetStateGraph(inst._data.were_sg or "SGwerepig")
    ...
    if inst._data.SetWerePigPost then
        inst._data.SetWerePigPost(inst)
    end
end
...
local function OnLoad(inst, data)
    if data ~= nil then
        inst.build = data.build or inst.build
...
local function MakePig(name, data, common_post, master_post)
    local builds = data.builds or data.is_guard and { "pig_guard_build" } or { "pig_build", "pigspotted_build" }
    if type(builds) == "string" then
        builds = { builds }
    end

    data.assets = data.assets or {}
    table.troinserttable(data.assets, assets)
    ...
    return Prefab(name, fn, data.assets, prefabs)
end
