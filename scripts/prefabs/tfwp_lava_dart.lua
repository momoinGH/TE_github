...
local function s(g, t, u, v, w, x)
    ...
    local y = -math.atan2(t.z - o, t.x - m) / DEGREES; if v and x ~= nil then
        local z = u.Transform:GetRotation()
        local A = y - z; y = Lerp(A > 180 and z + 360 or A < -180 and z - 360 or z, y, x * w)
    end; u.Transform:SetRotation(y)
end;
...
local function B(g, C, t)
    for D = 1, 6 do
        g:DoTaskInTime(0.08 * D, function()
            ...
            G.components.aimedprojectile:DelayVisibility(g.projectiledelay)
        end)
    end;
...
local function fn()
    ...
    inst.components.aoespell:SetAOESpell(B)
    ...
    inst.components.weapon:SetAltAttack(TUNING.FORGE_ITEM_PACK.TFWP_LAVA_DART.FORGE_ITEM_PACKDARTS, { 10, 20 }, nil, DAMAGETYPES.PHYSICAL)
    return inst
end;
local I = 5;
local function J()
...
local function N(g, O, h)
    ...
    g:Remove()
end; local function P(g, h)
    g:Remove()
end; local function Q(R)
    local
    g = CreateEntity()
    g.entity:AddTransform()
    g.entity:AddAnimState()
    ...
    if not R then
        ...
        g:DoTaskInTime(0, function(g) g.SoundEmitter:PlaySound("dontstarve/common/lava_arena/blow_dart") end)
    end;
    g.OnLoad = g.Remove; return g
end;
...
return
    CustomPrefab("tfwp_lava_dart", fn, a, c, nil, "images/inventoryimages.xml", "blowdart_lava.tex", TUNING.FORGE_ITEM_PACK.TFWP_LAVA_DART, "swap_blowdart_lava", "common_hand"),
