...
local function UpdateCampOccupied(inst)
    ...
    if inst.data.occupied then
        if not TheWorld.state.iswinter and ground ~= WORLD_TILES.SNOWLAND and ground ~= WORLD_TILES.ICELAND then
...
local function CheckSpawnHuntingParty(inst, target, houndsonly)
    ...
    local x, y, z = inst.Transform:GetWorldPosition()
    local ground = map:GetTile(map:GetTileCoordsAtPoint(x, y, z))
    if inst.data.occupied and TheWorld.state.iswinter or inst.data.occupied and ground == WORLD_TILES.SNOWLAND or inst.data.occupied and ground == WORLD_TILES.ICELAND then
