...
AddPrefabPostInit("player_classified",
    function(l)
        ...
        if not k:IsDedicated() then l:ListenForEvent("net_buffs_dirty",
                function(l)
                    if not l._parent or not l._parent.HUD or #l.net_buffs:value() < 1 then return end; l.net_buffs
                        :set_local("")
                    l.net_buffs:set("")
                end) end
    end)
...
AddComponentPostInit("combat", function(self)
    ...
    function self:AddDamageBuff(r, s, v) if not self:HasDamageBuff(r, s) then self.damagebuffs[r and "recieved" or "dealt"].generic[s] =
            v end end;
    ...
    function self:GetAttackRange()
        ...
        if z and z.components.weapon:CanAltAttack() then return z.components.weapon.altattackrange else return
            _oldGetAttackRange(self) end
    end;

    _oldGetHitRange = self.GetHitRange;
    function self:GetHitRange()
        local z = self:GetWeapon()
        if z and z.components.weapon:CanAltAttack() then return self.attackrange + z.components.weapon.althitrange else return
            _oldGetHitRange(self) end
    end;
    ...
    function self:DoSpecialAttack(C, T, x, V, a5)
        self.damage_override = C; if a5 then self:DoAreaAttack(a5.target, a5.range, nil, a5.validfn, x, a5.excludetags) else
            self:DoAttack(T, nil, nil, x, V) end; self.damage_override = nil
    end
end)
AddComponentPostInit("weapon",
    function(self)
        ...
        function self:SetAltAttack(X, Y, a6, q, a7, a8)
            ...
            end; if self.inst.replica.inventoryitem then self.inst.replica.inventoryitem:SetAltAttackRange(self
                .altattackrange) end
        end;

        function self:SetIsAltAttacking(aa, ab)
            self.isaltattacking = aa; if self.inst.replica.inventoryitem then self.inst.replica.inventoryitem
                    :SetIsAltAttacking(aa) end; if aa then
        ...
        function self:CanAltAttack() if self:HasAltAttack() and self.isaltattacking then return self.altcondition ~= nil and
                self.altcondition(self.inst) or true end end; _oldCanRangedAttack = self.CanRangedAttack;
            ...
            if G and G.components.combat and self:CanAltAttack() then if T and type(T) == "table" and not T.prefab then for S, y in ipairs(T) do
                        if y ~= G and y.entity:IsValid() and y.components.workable ~= nil and self.inst.IsWorkableAllowed ~= nil and type(self.inst.IsWorkableAllowed) == "function" and self.inst:IsWorkableAllowed(y.components.workable:GetWorkAction(), y) then
                            ...
                            if _G.TUNING.FORGE_ITEM_PACK[ac] ~= nil and _G.TUNING.FORGE_ITEM_PACK[ac].WORKABLE_DMG ~= nil then
                                y.components.workable:WorkedBy(G, _G.TUNING.FORGE_ITEM_PACK[ac].WORKABLE_DMG) end
                        end; if y ~= G and G.components.combat:IsValidTarget(y) and y.components.health and not y.components.health:IsDead() then
                            G.components.combat:DoAttack(y, self.inst, U, x, V) end
                    end else if G.components.combat:IsValidTarget(T) then G.components.combat:DoAttack(T, self.inst, U, x,
                            V) end end end; G:PushEvent("alt_attack_complete", { weapon = self.inst })
...
AddComponentPostInit("equippable",
    function(self)
        ...
        function self:AddHealthBuff(s, v) if not self:HasHealthBuff(s) then
                local ae = self.inst.components.inventoryitem.owner; if ae.components.health then ae.components.health
                        :AddHealthBuff(s, v) end
                ...
                self.healthbuffs[s] = nil; local ae = self.inst.components.inventoryitem.owner; if ae.components.health then
                    ae.components.health:RemoveHealthBuff(s) end
            end end;
        ...
        function self:AddDamageBuff(r, s, v) if not self:HasDamageBuff(r, s) then
                self.damagebuffs[r and "recieved" or "dealt"].generic[s] = v; if self:IsEquipped() then
                    local ae = self.inst.components.inventoryitem.owner; if ae.components.combat then ae.components
                            .combat:AddDamageBuff(r, s, v) end
                end
            end end;
        ...
        function self:AddDamageTypeBuff(r, w, s, v) if not self:HasDamageBuff(r, s, w) then
                ...
                v; if self:IsEquipped() then
                    local ae = self.inst.components.inventoryitem.owner; if ae.components.combat then ae.components
                            .combat:AddDamageTypeBuff(r, w, s, v) end
                end
            end end; function self:AddDamageStimuliBuff(r, x, s, v) if not self:HasDamageBuff(r, s, x) then
                local u = self.damagebuffs[r and "recieved" or "dealt"].stimuli; if not u[x] then u[x] = {} end; u[x][s] =
                v; if self:IsEquipped() then
                    local ae = self.inst.components.inventoryitem.owner; if ae.components.combat then ae.components
                            .combat:AddDamageStimuliBuff(r, x, s, v) end
                end
            end end;
        ...
        function self:RemoveDamageBuff(r, s, t) if self:HasDamageBuff(r, s, t) then
                ...
                if not t then u[s] = nil else
                    ...
                    local ae = self.inst.components.inventoryitem.owner; if ae.components.combat then ae.components
                            .combat:RemoveDamageBuff(r, s, t) end
                end
            end end;

        function self:AddBuffsTo(ae) if ae and ae.components.buffable then ae.components.buffable:AddBuff(
                self.inst.prefab, self.uniquebuffs) end end;

        function self:RemoveBuffsFrom(ae) if ae and ae.components.buffable then ae.components.buffable:RemoveBuff(self
                .inst.prefab) end end;
        ...
        function self:Equip(ae)
            if ae.components.combat then for P, af in pairs(self.damagebuffs) do for o, K in pairs(af) do for t, v in pairs(K) do if o == "generic" then
                                ae.components.combat:AddDamageBuff(P == "recieved", t, v) else for ah, ai in pairs(v) do if o == "dmgtype" then
                                        ae.components.combat:AddDamageTypeBuff(P == "recieved", t, ah, ai) else ae
                                            .components.combat:AddDamageStimuliBuff(P == "recieved", t, ah, ai) end end end end end end end; self
                :AddBuffsTo(ae)
            ag(self, ae)
        end; local aj = self.Unequip;
        function self:Unequip(ae)
            if ae.components.combat then for P, af in pairs(self.damagebuffs) do for o, K in pairs(af) do for t, v in pairs(K) do if o == "generic" then
                                ae.components.combat:RemoveDamageBuff(P == "recieved", t) else for s, S in pairs(v) do ae
                                        .components.combat:RemoveDamageBuff(P == "recieved", s, t) end end end end end end; self
                :RemoveBuffsFrom(ae)
            aj(self, ae)
        end
    end)
AddClassPostConstruct("components/inventoryitem_replica",
    function(self)
        self.can_altattack_fn = function() return true end; function self:SetIsAltAttacking(aa) self.classified
                .isaltattacking:set(aa) end; function self:SetAltAttackRange(Y) self.classified.altattackrange:set(Y or
            self:AttackRange()) end; function self:AltAttackRange() if self.inst.components.weapon then return self.inst
                .components.weapon.altattackrange elseif self.classified ~= nil then return math.max(0,
                    self.classified.altattackrange:value()) else return self:AttackRange() end end;
        ...
        function self:SetAltAttackCheck(ak) if ak and type(ak) == "function" then self.can_altattack_fn = ak end end; function self:CanAltAttack() if self.classified.isaltattacking:value() then return
                self.inst.components.weapon and self.components.weapon:CanAltAttack() or self.can_altattack_fn(self.inst) end end; _oldAttackRange =
        self.AttackRange; function self:AttackRange() return self:CanAltAttack() and self:AltAttackRange() or
            _oldAttackRange(self) end
    end)
...
AddComponentPostInit("debuffable",
    function(self)
        local as = self.AddDebuff; local at = self.RemoveDebuff; function self:AddDebuff(m, ...)
            if self.debuffs[m] == nil and self.inst.player_classified then self.inst.player_classified.net_buffs:set(a
                .encode({ name = m, removed = false })) end; as(self, m, ...)
        end;

        function self:RemoveDebuff(m, ...)
            if self.debuffs[m] ~= nil and self.inst.player_classified then self.inst.player_classified.net_buffs:set(a
                .encode({ name = m, removed = true })) end; at(self, m, ...)
        end
    end)
...
local ac = function(...) end; for av, aw in pairs(au) do AddStategraphPostInit("wilson",
        function(self)
            local ax, ay, az = self.states[av].onenter or ac, self.states[av].onexit or ac,
                ...
                if not l.sg.statemem.equip then return end; if l.sg.statemem.tm < l.sg.statemem.trgt and l.sg.statemem.equip.components.reticule_spawner then
                    l.sg.statemem.equip.components.reticule_spawner:Interrupt() end; ay(l)
            end; self.states[av].onupdate = function(l, aA)
                l.sg.statemem.tm = l.sg.statemem.tm + aA; az(l)
            end
        end) end; local aB = { "spider", "spider_warrior", "tentacle", "hound", "firehound", "icehound", "krampus",
    ...
    "buzzard", "mossling", "antlion", "gestalt", "gnarwail", "gnarwail_attack_horn", "fruitdragon", "squid", "player" }
for S, aC in ipairs(aB) do AddPrefabPostInit(aC,
        function(l) if l.components and l.components.combat then l.components.combat:SetDamageType(1) end end) end
