...
-- 绘制房间和房间间的箭头
local function DrawPassage(root, dir, width, depth, room_offset_x, room_offset_y)
    ...
    local passage = AddImage(root, "levels/textures/tro_map_interior/passage.xml", "passage.tex", room_offset_x + dx, room_offset_y + dy)
    if rotate then
...
local function AppendRoomTexture(self, start_room, room_offset_x, room_offset_y)
    ...
    for grid_x, row in ipairs(room_grid) do
        for grid_y, room_data in ipairs(row) do
            if room_data then
                room_offset_x = (grid_y - start_y) * (width * ratio + 18)
                room_offset_y = (start_x - grid_x) * (depth + 18)
                ...
                --检查每个房间右边和上面有没有房间，绘制箭头
                if room_data.north then
                    table.insert(passage_image, DrawPassage(room_root, "north", width, depth, room_offset_x, room_offset_y))
                end
                if room_data.east then
                    table.insert(passage_image, DrawPassage(room_root, "east", width, depth, room_offset_x, room_offset_y))
                end
            end
        end
    end
